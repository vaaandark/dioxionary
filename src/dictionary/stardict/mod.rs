...
impl<'a> StarDict {
    ...
    pub fn new<P: AsRef<Path>>(dir_path: P) -> Result<StarDict> {
        ...
        let mut indices = DictionaryIndices::new(indices.unwrap(), metadata.version())?;
        let contents = DictionaryContents::new(contents.unwrap())?;
    ...
    pub fn exact_look_up(&self, word: &str) -> Option<DictionaryEntry<'_>> {
        if let Ok(pos) = self.indices.items.binary_search_by(|probe| {
            ...
            Some(DictionaryEntry { word, translation })
        } else {
            None
        }
    }
    ...
    pub fn fuzzy_look_up(&self, word: &str) -> Option<Vec<DictionaryEntry<'_>>> {
        ...
        let result = self
            ...
            .map(|(_, x)| {
                ...
                let translation = self.contents.get(*offset, *size);
                DictionaryEntry { word, translation }
            })
            .collect::<Vec<_>>();
        Some(result)
    }
...
struct DictionaryContents(String);

#[allow(unused)]
impl<'a> DictionaryContents {
    fn new<P: AsRef<Path>>(path: P) -> Result<DictionaryContents> {
        let path = path.as_ref();
        let s =
        ...
        Ok(DictionaryContents(contents))
    }
...
impl DictionaryIndices {
    ...
    fn new<P: AsRef<Path>>(path: P, version: StarDictVersion) -> Result<DictionaryIndices> {
        let path = path.as_ref();
        match version {
            StarDictVersion::V2_4_2 => Ok(DictionaryIndices {
                items: DictionaryIndices::read_bytes::<u32, { mem::size_of::<u32>() }, _>(path)?,
            }),
            StarDictVersion::V3_0_0 => Ok(DictionaryIndices {
                items: DictionaryIndices::read_bytes::<u64, { mem::size_of::<u64>() }, _>(path)?,
            }),
            StarDictVersion::Unknown => {
